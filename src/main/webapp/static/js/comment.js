    ...
    $.fn.zyComment = function(options,param){
        ...
        return this.each(function(){
            ...
            this.createAgoCommentHtml = function(){
                ...
                var html = '';
                html += '<div id="commentItems" class="comments">';
                html += '	<div class="text" > 来评论一下吧! </div>';
                html += '</div>';
                $(self).append(html);

                var stringId = [];
                $.each(para.agoComment, function(k, v){
                    stringId[v.id] ="评论"+"<span>"+v.userName+"</span>";
                });
                stringId[0]="";
                ...
                $.each(para.agoComment, function(k, v){
                    ...
                    item += '	<a class="avatar">';
                    item += '		<img src='+v.headPortrait+ '>';
                    item += '	</a>';
                    item += '	<div class="content '+topStyle+'">';
                    item += '		<a class="author"> '+'<span>'+v.userName+'</span>'+stringId[v.sortID]+":"+' </a>';
            ...
            this.createFormCommentHtml = function(){
                ...
                // 组织发表评论的form html代码
                var boxHtml = '';
                boxHtml += '<form id="replyBoxAri" class="reply-form">';
                boxHtml += '	<div class="large-form ">';

                boxHtml += '		<div class="contentField" >';
                boxHtml += '			<textarea id="commentContent"></textarea>';

                boxHtml += '		</div>';
                boxHtml += '		<div id="submitComment" class="button button-primary button-rounded button-small submit">';
                boxHtml += '			提交评论';
                boxHtml += '		</div>';
                boxHtml += '	</div>';
                boxHtml += '</form>';
            ...
            this.addReplyCommentFrom = function(id){
                var boxHtml = '';
                boxHtml += '<form id="replyBox" class="reply-form">';
                boxHtml += '	<div class="small-form ">';

                boxHtml += '		<div class="contentField" >';
                boxHtml += '			<textarea id="commentContent"></textarea>';

                boxHtml += '		</div>';
                boxHtml += '		<div id="publicComment" class="button button-primary button-rounded button-small submit">';
                boxHtml += '			提交评论';
                boxHtml += '		</div>';
                boxHtml += '	</div>';
                boxHtml += '</form>';
            ...
            this.addNewComment = function(param){
                ...
                item += '	<a class="avatar">';
                item += '		<img src='+param.headPortrait+ '>';
                item += '	</a>';
                item += '	<div class="content '+topStyle+'">';
                item += '		<a class="author"> '+'<span>'+param.name+'</span>'+stringId[fCode]+":"+' </a>';
                item += '		<div class="metadata">';
